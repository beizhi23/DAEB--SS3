...
class SS3:
    ...
    def extract_insight(
        self, a,doc, cat='auto', level='word', window_size=3, min_cv=0.01, sort=True,
    ):
        """
        获取分类决策中涉及的文本块列表。
        ...
        :raises: InvalidCategoryError, ValueError
        """
        r = self.classify(a,doc, json=True,)
    ...
    def classify(self,a:float,doc, prep=True, sort=True, json=False, prep_func=None,):
        """
       对给定文档进行分类。
    ...
    def classify_label(self,a,doc, def_cat=STR_MOST_PROBABLE, labels=True, prep=True):
        """
        对返回类别标签的给定文档进行分类。
        ...
        :raises: InvalidCategoryError
        """
        r = self.classify(a,doc,sort=True, prep=prep)
    ...
    def classify_multilabel(self,a, doc, def_cat=STR_UNKNOWN, labels=True, prep=True):
        """
       对返回多个类别标签的给定文档进行分类。
    ...
    def predict_proba(self,a,x_test, prep=True, leave_pbar=True):
        """
       对返回置信向量列表的文档列表进行分类。
    ...
    def predict(
       ...
       对文档列表进行分类。
        ...
        y_pred = [
            classify(a, doc, def_cat=def_cat, labels=labels, prep=prep)
